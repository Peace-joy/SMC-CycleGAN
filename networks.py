...
from advance.SAFM2 import MultiScaleFeatureExtractor
from advance.self import SelfAttention
...
class MultiScaleResnetBlock(nn.Module):
    """ResNet Block with Multi-Scale Feature Extraction"""
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        super(MultiScaleResnetBlock, self).__init__()
        self.multi_scale_conv = MultiScaleFeatureExtractor(dim)
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)

    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        """Build the standard ResNet convolution block"""
        conv_block = []
        p = 0
        if padding_type == 'reflect':
            conv_block += [nn.ReflectionPad2d(1)]
    ...
    def forward(self, x):
        ...
        residual_output = self.conv_block(multi_scale_features)
        out = x + residual_output
        return out
